...
public class JavaSphincsplus {
	...
	System.out.println("secret key: " + bytesToHex(sk));

	int SPX_SIGNATURES = 1;
	System.out.printf("\nTesting %d signatures.. \n", SPX_SIGNATURES);
	int SPX_MLEN = 32;
	byte[] m = new byte[SPX_MLEN];
	for (int i=0; i <  m.length; i++)
	    m[i] = 1;

	System.out.println("  - iteration #0:");
	byte[] sm = crypto_sign(m, sk);
	if (sm.length == SPX_BYTES + m.length)
	    System.out.println("    smlen as expected [" + sm.length + "].");
	System.out.println("signature[0-70]: " + bytesToHex(Arrays.copyOfRange(sm, 0, 70)));
	System.out.println("signature[last 70 of sig]: " + bytesToHex(Arrays.copyOfRange(sm, SPX_BYTES - 70, SPX_BYTES)));
	//for (int i=0; i < (sm.length + 63)/64;  i++)
	//    System.out.println(bytesToHex(Arrays.copyOfRange(sm, i*64, Math.min((i+1)*64, sm.length))));

	byte[] unsignedm = crypto_sign_open(sm, pk);
	if (!Arrays.equals(unsignedm, m))
	    throw new IllegalStateException("Incorrect unsiged message!");
	System.out.println("signature verified correctly!");

	System.out.printf("Checking a changed signature is invalid.");
	sm[0] ^= 0x1;
	try {
	...
	System.out.printf("Checking signature is invalid for a changed message.");
	sm[0] ^= 1;
	sm[sm.length-1] ^= 1;
	try {
	    crypto_sign_open(sm, pk);
    ...
    private static void benchmark(byte[] m, byte[] pk, byte[] sk) {
        int res = 0;
	...
	//System.out.println("crypto_sign.sig0: " + bytesToHex(Arrays.copyOfRange(sig, 0, 70)));

	/* Derive the message digest and leaf index from R, PK and M. */
	hash_message(mhash, tree, idx_leaf, sig, pk, m, ctx);
	//System.out.println("crypto_sign.mhash: " + bytesToHex(mhash));
	int sigOffset = SPX_N;
	//System.out.println("crypto_sign.sig: " + bytesToHex(Arrays.copyOfRange(sig, 0, 70)));

	//System.out.println("crypto_sign.wots_addr: " + Arrays.stream(wots_addr).mapToObj(x -> x).collect(Collectors.toList()));
	//System.out.println("crypto_sign.tree: " + tree[0]);
	set_tree_addr(wots_addr, tree[0]);
	//System.out.println("crypto_sign.wots_addr: " + Arrays.stream(wots_addr).mapToObj(x -> x).collect(Collectors.toList()));
	set_keypair_addr(wots_addr, idx_leaf[0]);
	//System.out.println("crypto_sign.idx_leaf: " + idx_leaf[0]);
	//System.out.println("crypto_sign.wots_addr: " + Arrays.stream(wots_addr).mapToObj(x -> x).collect(Collectors.toList()));
	
	/* Sign the message hash using FORS. */
	fors_sign(sig, sigOffset, root, mhash, ctx, wots_addr);
	//System.out.println("crypto_sign.sig2: " + bytesToHex(Arrays.copyOfRange(sig, sigOffset, sigOffset + 70)));
	...
	for (i = 0; i < SPX_D; i++) {
	    ...
	    //System.out.println("crypto_sign.root0: " + bytesToHex(root));
	    //System.out.println("crypto_sign.idx_leaf4: " + idx_leaf[0]);
	    //System.out.println("crypto_sign.wots_addr4: " + Arrays.stream(wots_addr).mapToObj(x -> x).collect(Collectors.toList()));
	    //System.out.println("crypto_sign.tree_addr4: " + Arrays.stream(tree_addr).mapToObj(x -> x).collect(Collectors.toList()));
	    merkle_sign(sig, sigOffset, root, 0, ctx, wots_addr, tree_addr, idx_leaf[0]);
	    //System.out.println("crypto_sign.root: " + bytesToHex(root));
	    //System.out.println("crypto_sign.sig3: " + bytesToHex(Arrays.copyOfRange(sig, sigOffset, sigOffset + 70)));
	...
	//System.out.println("gen_message_random.buf: " + bytesToHex(buf));
	...
	//System.out.println("hash_message.bufraw: " + bytesToHex(Arrays.copyOfRange(buf, bufp, bufp+8)));
	//System.out.println("hash_message.tree: " + tree[0]);
	...
	message_to_indices(indices, m);
	//System.out.println("fors_sign.indices: " + Arrays.stream(indices).mapToObj(x -> x).collect(Collectors.toList()));
	//System.out.println("fors_sign.fors_tree_addr: " + Arrays.stream(fors_tree_addr).mapToObj(x -> x).collect(Collectors.toList()));
	...
	for (i = 0; i < SPX_FORS_TREES; i++) {
	    ...
	    //System.out.println("fors_sign.fors_tree_addr2: " + Arrays.stream(fors_tree_addr).mapToObj(x -> x).collect(Collectors.toList()));
	    fors_gen_sk(sig, sigOffset, ctx, fors_tree_addr);
	    //System.out.println("fors_sign.sig0: " + bytesToHex(Arrays.copyOfRange(sig, sigOffset, sigOffset + 70)));
	    set_type(fors_tree_addr, SPX_ADDR_TYPE_FORSTREE);
	    sigOffset += SPX_N;
	    ...
	    treehashx1(roots, i*SPX_N, sig, sigOffset, ctx,
		       ...
		       fors_tree_addr);
	    //System.out.println("fors_sign.sig: " + bytesToHex(Arrays.copyOfRange(sig, sigOffset, sigOffset + 70)));
	...
	//System.out.println("fors_gen_leafx1.fors_leaf_addr: " + Arrays.stream(fors_leaf_addr).mapToObj(x -> x).collect(Collectors.toList()));
	//System.out.println("fors_gen_leafx1.leaf: " + bytesToHex(Arrays.copyOfRange(leaf, leafOffset, leafOffset + 10)));
	
	set_type(fors_leaf_addr, SPX_ADDR_TYPE_FORSTREE);
	fors_sk_to_leaf(leaf, leafOffset, leaf, leafOffset, ctx, fors_leaf_addr);
	//System.out.println("fors_gen_leafx1.leaf2: " + bytesToHex(Arrays.copyOfRange(leaf, leafOffset, leafOffset + 10)));
    }
	...
	//System.out.println("wots_gen_leafx1.leaf_addr[in]: " + Arrays.stream(leaf_addr).mapToObj(x -> x).collect(Collectors.toList()));
	...
	for (i = 0, buffer = pk_buffer; i < SPX_WOTS_LEN; i++, bufferOffset += SPX_N) {
	    ...
	    //System.out.println("wots_gen_leafx1.leaf_addrXXXXX: " + Arrays.stream(leaf_addr).mapToObj(x -> x).collect(Collectors.toList()));
	    set_chain_addr(leaf_addr, i);
	    set_hash_addr(leaf_addr, 0);
	    //System.out.println("wots_gen_leafx1.leaf_addrYYYYY: " + Arrays.stream(leaf_addr).mapToObj(x -> x).collect(Collectors.toList()));
	    set_type(leaf_addr, SPX_ADDR_TYPE_WOTSPRF);
	    //System.out.println("wots_gen_leafx1.leaf_addr: " + Arrays.stream(leaf_addr).mapToObj(x -> x).collect(Collectors.toList()));
	    
	    prf_addr(buffer, bufferOffset, ctx, leaf_addr);
	    //System.out.println("wots_gen_leafx1.buf: " + bytesToHex(Arrays.copyOfRange(buffer,  bufferOffset, bufferOffset + SPX_N)));
	...
	//System.out.println("wots_gen_leafx1.pk_buffer: " + bytesToHex(pk_buffer));
	//System.out.println("wots_gen_leafx1.pk_addr: " + Arrays.stream(pk_addr).mapToObj(x -> x).collect(Collectors.toList()));
	...
	for (idx = 0;; idx++) {
	    ...
	    //System.out.println("current: " + bytesToHex(current));
		...
		if (h == tree_height) {
		    ...
		    //System.out.println("treehashx1.root: " + bytesToHex(Arrays.copyOfRange(root, rootOffset, rootOffset + SPX_N)));
		    return;
		}
		...
		//System.out.println("treehashx1.tree_addr(0): " + Arrays.stream(tree_addr).mapToObj(x -> x).collect(Collectors.toList()));
		set_tree_height(tree_addr, h + 1);
		//System.out.println("treehashx1.tree_addr(1): " + Arrays.stream(tree_addr).mapToObj(x -> x).collect(Collectors.toList()));
		set_tree_index(tree_addr, internal_idx/2 + internal_idx_offset );
		//System.out.println("treehashx1.tree_addr(2): " + Arrays.stream(tree_addr).mapToObj(x -> x).collect(Collectors.toList()));
		
		System.arraycopy(stack, h * SPX_N, current, 0, SPX_N);
		//System.out.println("current(hloop5): " + bytesToHex(current));
		//System.out.println("treehashx1.tree_addr: " + Arrays.stream(tree_addr).mapToObj(x -> x).collect(Collectors.toList()));
		thash(current, 1 * SPX_N, current, 0, 2, ctx, tree_addr);
		//System.out.println("current(hloop): " + bytesToHex(current));
	    }
	...
	//System.out.println("thash.in: " + bytesToHex(Arrays.copyOfRange(in, inOffset, inOffset + inblocks * SPX_N)));
	//System.out.println("thash.addr: " + Arrays.stream(addr).mapToObj(x->x).collect(Collectors.toList()));
	//System.out.println("thash.ctx.pub_seed: " + bytesToHex(ctx.pub_seed));
	...
	//System.out.println("thash.bitmask: " + bytesToHex(bitmask));
	...
	Sha256 res = new Sha256(sha2_state, 64);
	//System.out.println("thash.buf: " + bytesToHex(Arrays.copyOfRange(buf, SPX_N, SPX_N + SPX_SHA256_ADDR_BYTES + inblocks*SPX_N)));
